...
- The modules in this role use Central's REST API. For information on REST API and how to obtain an access for using REST API, visit the Aruba Developer Hub: [Getting Started with REST API](https://developer.arubanetworks.com/aruba-central/docs/getting-started)
...
- The **`refresh_token`** along with the **`client_id`** and **`client_secret`** are used to renew the access token. This functionality is implemented in this role by using an Inventory Plugin.
- More details on how to use the tokens and other credentials in an inventory plugin config file for auto-renew of tokens is mentioned below in the [Inventory Plugin Config File](https://github.com/aruba/aruba-central-ansible-role#inventory-plugin-config-variables).
...
### Caveats
- Inventory Plugin can not be shipped within a role since Ansible executes the Inventory Plugin before execution of the Playbook or Role.  More information can be found on [Ansible Docs](https://docs.ansible.com/ansible/latest/dev_guide/developing_inventory.html#inventory-plugins).
	- Therefore, until we publish the Aruba Central Ansible Collection, you need to perform the following workaround for using custom inventory plugin for Central - which will take care of the auto-renew of tokens.
...
```
- Where `inv_src.yml` or any other `.yml` file with a different name acts as the Inventory Plugin Config file. Sample inventory plugin config file is given below.
